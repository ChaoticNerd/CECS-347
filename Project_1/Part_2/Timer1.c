...
void Timer1A_Init(unsigned long period){
  ...
  TIMER1_CFG_R = 0x00000100;    // 2) configure for 16-bit mode
  TIMER1_TAMR_R = 0x00000002;   // 3) configure for periodic down-count mode
  TIMER1_TAILR_R = period-1;    // 4) reload value
  TIMER1_TAPR_R = 8;            // 5) bus clock prescale
  TIMER1_ICR_R = 0x00000010;    // 6) clear TIMER0A timeout flag
  TIMER1_IMR_R = 0x00000010;    // 7) arm timeout interrupt
...
// vector number 35, interrupt number 19
  ...
  TIMER1_CTL_R = 0x00000010;    // 10) enable TIMER0A
}
