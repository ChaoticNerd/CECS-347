...
#include "Nokia5110.h"
#include "stdint.h"
#include "stdio.h" // used to convert number to string
...
// outputs: none
// assumes: LCD is in default horizontal addressing mode (V = 0)
void Nokia5110_OutString(unsigned char *ptr){  //"cecs347"
  while(*ptr){
...
// Inputs: n  16-bit unsigned number: 0 to 2^16-1=65535
// Outputs: none
// assumes: LCD is in default horizontal addressing mode (V = 0)
void Nokia5110_OutUDec(unsigned short n){
	volatile unsigned long index; // number position in string
	unsigned short sub_n = n;   // used to find length of number
	uint8_t digit;              // number to sent to char
	uint8_t count = 0;          // length of number
	if (n==0){                  // when n=0 rest of the code won't run when n is 0 so the if catches it
		Nokia5110_OutString((unsigned char *)" ");
		Nokia5110_OutChar(n+'0');
	}
	while (sub_n != 0){        // find length
		sub_n = sub_n / 10;
		count += 1;
	}
	char num_str[count];         // create array to hold string
	sprintf(num_str, "%d", n);   // convert value to string
	for(index=0; index<count; index=index+1){  // loop through string in array pulling out each digit to send
		digit = num_str[index];
		Nokia5110_OutChar(digit);
	}
}
//********Nokia5110_SetCursor*****************
