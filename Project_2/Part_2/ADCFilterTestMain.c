...
int main(void){	
  PLL_Init();                         // 16 MHz, 
	...
	// these enable one ADC, we need 3 total ::: these are all software ADC 
//  ADC0_InitSWTriggerSeq3_Ch1();     // ADC initialization PE2/AIN1
	ADC0_InitSWTriggerSeq3_Ch7();       // ADC initialization PD0/AIN7 
	
  while(1){
    GPIO_PORTF_DATA_R |= 0x04;          // Hardware Debug (led on)
    ADCvalue = ReadADCMedianFilter();
		Delay();
    GPIO_PORTF_DATA_R &= ~0x04;					// Hardware Debug (led off)
  }
}
