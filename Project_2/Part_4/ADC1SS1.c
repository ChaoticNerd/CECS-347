...
void ADC1SS3_Init(void){ 
  ...
  ADC1_PC_R &= ~0x0F;              // 8) clear max sample rate field (hidden leading 0s)
  ADC1_PC_R |= 0x01;               //    configure for 125K samples/sec (hidden leading 0s)
  ...
  ADC1_SSMUX3_R = 0x08; 						// 12) set channels for SS3 MUX 0		
